  ...
  useEffect(() => {
    ...
    async function fetchSettings() {
      try {
        ...
        setMarqueeText(data.marqueeText || 'Welcome to NB SAFA AGRO! Premium High-Starch Maize Silage Feed Production.');
      } catch (error: any) {
        if (error.name !== 'AbortError') {
          console.error('Error fetching marquee text:', error);
          setMarqueeText('Welcome to NB SAFA AGRO! Premium High-Starch Maize Silage Feed Production.');
        }
      }
    }
    fetchSettings();
