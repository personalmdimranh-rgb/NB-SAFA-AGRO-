  ...
  try {
    ...
    if (sessionUser?.user?.id) {
      ...
      if (user) {
        ...
        const newUserList = (await User.create([{
    ...
    // 2d. Price Verification and Validated Items List
    for (const item of items) {
    ...
    // If it's Online, it might start as Pending until callback confirms it, but let's allow it to be configured
    ...
    if (session) {
      try {
        ...
        console.error('session.abortTransaction failed, attempting manual rollback fallback:', abortErr);
        abortFailed = true;
      }
    } else {
      abortFailed = true;
    ...
    if (abortFailed && successfulDeductions.length > 0) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    const isAdmin = ['admin', 'super_admin', 'manager', 'staff'].includes((session.user as any)?.role);
