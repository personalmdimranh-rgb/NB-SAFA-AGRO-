...
export async function GET(
  ...
  try {
    ...
    // Authorization: Must be an admin/staff OR the owner of the order
...
export async function PATCH(
  ...
  try {
    const { slug } = await params;
    const session = await auth();
    if (!session || !session.user || !(['admin', 'super_admin', 'manager', 'staff'].includes((session.user as any)?.role))) {
      return NextResponse.json({ message: 'Forbidden' }, { status: 403 });
    }
...
export async function DELETE(
  ...
  try {
    ...
    // Delete corresponding ledger entry if any
    const orderShortId = order.shortId || order._id.toString().slice(-8).toUpperCase();
    await LedgerTransaction.deleteMany({ description: `Payment for order #${orderShortId}` });

    await Order.findOneAndDelete({ _id: slug });
