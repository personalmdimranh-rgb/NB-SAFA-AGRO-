...
export default function FarmerPlaceOrder() {
  ...
  const handleAddItem = () => {
    setItems([...items, { productName: 'Premium Silage Bag (40kg)', productType: 'bag', quantity: 5, unitPrice: 380 }]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const confirmResult = await Swal.fire({
      ...
      html: `Order total: ৳${grandTotal.toLocaleString()} BDT.<br/>Dues logged: ৳${dueAmount.toLocaleString()} BDT.`,
      icon: 'question',
  ...
  if (!farmer) {
    return (
      <div className="text-center py-20 text-rose-500">
        Farmer account details not found. Please contact administration to register your profile.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground font-semibold">
          Customer Portal: <span className="text-primary">{farmer.name} ({farmer.phone})</span>
        </p>
      </div>
