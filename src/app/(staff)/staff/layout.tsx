...
export default async function StaffLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-zinc-50 p-4">
        <div className="max-w-md w-full text-center bg-white p-6 rounded-xl border border-rose-200 shadow-sm space-y-4">
          ...
          <h2 className="text-xl font-bold text-rose-950">Staff Account Inactive</h2>
          <p className="text-sm text-muted-foreground">Your account is registered but pending administrative approval. Please contact support.</p>
  ...
  const employeeProfile = await Employee.findOne({ email: dbUser.email });
      ...
      <aside className="w-full md:w-64 md:h-full bg-primary text-primary-foreground flex flex-col justify-between shrink-0 p-4 md:overflow-y-auto">
        <div className="space-y-6">
          <div className="p-2 border-b border-primary-foreground/20 space-y-3">
            ...
            />
            <div>
              <p className="text-[10px] text-primary-foreground/70 font-semibold uppercase">Staff Portal</p>
              <p className="text-xs text-primary-foreground/90 font-bold truncate mt-1">{employeeProfile?.name || dbUser.name}</p>
            </div>
          </div>

          <nav className="space-y-1">
            <Link href="/staff/dashboard" className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm font-semibold hover:bg-primary-foreground/10 transition-colors">
